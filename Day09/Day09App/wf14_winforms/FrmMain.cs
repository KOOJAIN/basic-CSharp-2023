...
namespace wf14_winforms
{
    public partial class FrmMain : Form
    {
        int number = 0; //전체 변수
        int percent = 0;    // 나중에 만들거?
        public FrmMain()
        {
            InitializeComponent();
        }
        ...
        {
            Worker.WorkerReportsProgress = true;    // 중간에 작업을  진행할 것이냐
            Worker.WorkerSupportsCancellation = true;   // 중간에 작업을 취소할 것이냐
        }

        //백그라운드로 일 진행 // Tread.Start()
        private void Worker_DoWork(object sender, DoWorkEventArgs e)
        {
            BackgroundWorker worker = sender as BackgroundWorker;       // 인자값으로 BackgroundWorker 생성 얕은복사 - Worker랑 주소 참조

            e.Result = Fibonacci((int)e.Argument, worker, e); // 피보나치 계산 메서드
        }

        private long Fibonacci(int arg, BackgroundWorker worker, DoWorkEventArgs e)        // long
        {
            // 파라미터   arg는 0~91 사이 91보다 크면 long 에서 오버플로우 발생
            if (arg <0 || arg > 91)
            {
