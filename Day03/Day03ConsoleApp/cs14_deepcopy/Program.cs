...
namespace cs14_deepcopy
{
    class SomeClass
    {
        public int SomeField1; // {  get; set;          // 나중에 java에서는 '게터세터', c#에서는 '겟셋'이라 부름. 디폴드 값이기때문에 나온다고한다.
        ...
        public SomeClass DeepCopy()
        {
            ...
            return newCopy; //원래있는 값을 복사해서 리턴해서 받는 메소드
        }
    }
    ...
    class Employee
    {
        ...
        {
            this.Name = Name;   //클레스 맴버변수(속성)과 메서드의 매개변수이름이 대소문자까지 완전히 똑같을때 .
        }
    }
...
//객체 지향의 4가지 특성
// 캡슐화 - 데이터를 은닉하고 그 데이터를 접근하는 기능을 노출시키지 않는다는 의미
// 상속
// 추상화
// 다형성 - 오버라이딩, 오버로딩
